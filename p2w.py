...
# ==========================================
# TAB 2: XỬ LÝ OFFLINE (TÍCH HỢP XỬ LÝ ZIP CỦA P2W-COPY VÀ XỬ LÝ RIÊNG JSON/MD ĐƠN LẺ)
# ==========================================
with tab_offline:
    st.subheader("📁 Chuẩn hóa cấu trúc ZIP thô & Dựng file Word")
    st.markdown("💡 *Tải lên file ZIP nguồn, hoặc file JSON/Markdown đơn lẻ (kèm upload ảnh bổ sung bên dưới nếu cần).*")
    ...
    # Thêm tính năng upload ảnh riêng lẻ cho file JSON/Markdown đơn độc đúng như hình bạn yêu cầu
    offline_images = st.file_uploader(
        "🖼️ Tải kèm hình ảnh bổ sung (dùng riêng cho file JSON/Markdown đơn lẻ)",
    ...
    if normalize_zip_btn or build_word_btn:
        if not offline_file:
            ...
            # Nạp ảnh bổ sung từ vùng upload riêng (nếu có)
            if offline_images:
            ...
            try:
                ...
                # 1. Xử lý khi nạp file ZIP chuẩn
                ...
                # 2. Xử lý riêng cho file JSON đơn lẻ
                ...
                # 3. Xử lý riêng cho file Markdown đơn lẻ (.md)
                ...
                # Cơ chế làm sạch & chuẩn hóa văn bản
                ...
                # Chức năng chuẩn hóa cấu trúc ZIP
                if normalize_zip_btn:
                    ...
                    st.success(f"✅ Đã chuẩn hóa xong! Tìm thấy {len(images_dict)} ảnh. Bạn có thể tải file ZIP sạch bên dưới.")
                    st.download_button(
                ...
                # Chức năng dựng file Word ngay lập tức
                if build_word_btn:
                    compile_markdown_to_word(full_markdown, images_dict)
                    st.success(f"🎉 Đã bóc tách {len(images_dict)} ảnh và dựng xong file Word thành công!")
            except Exception as e:
                log_error(f"Lỗi chuẩn hóa file offline: {str(e)}")
